...
def information_gain(Y, attr, criterion=None):
    ...
    if (check_if_real(Y) and check_if_real(attr)):
    ...
    elif (check_if_real(attr) and (not check_if_real(Y)) and criterion == "information_gain"):
        # Create a table with 'attribute' and 'output' columns
        table = pd.concat([attr, Y], axis=1).reindex(attr.index)
        table.columns = ['attribute', 'output']
        table.sort_values(by='attribute', inplace=True)
    ...
    elif (check_if_real(attr) and (not check_if_real(Y)) and criterion == "gini_index"):
        # Create a table with 'attribute' and 'output' columns
        table = pd.concat([attr, Y], axis=1).reindex(attr.index)
        table.columns = ['attribute', 'output']
        table.sort_values(by='attribute', inplace=True)
    ...
    elif ((not check_if_real(attr)) and (not check_if_real(Y)) and criterion == "information_gain"):
        parent_entropy = entropy(Y)
        total_size = Y.size
    ...
    elif ((not check_if_real(attr)) and (not check_if_real(Y)) and criterion == "gini_index"):
        parent_gini = gini_index(Y)
        total_size = Y.size
        classes = np.unique(attr)
        child_gini = 0
    ...
    elif ((not check_if_real(attr)) and check_if_real(Y)):
        parent_var = np.var(Y)
        total_size = Y.size
        classes = np.unique(attr)
        child_var = 0
